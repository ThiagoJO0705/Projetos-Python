...
@customers.get('/me')
async def get_customer(user: dict = Depends(get_or_create_pyinvest_user)):
    '''Retorna os dados do investidor logado e, caso seja o primeiro acesso vindo do Banco Javer, realiza o auto-cadastro.'''
    return user['pyinvest']

@customers.patch('/me')
async def update_customer(update_data: CustomerUpdate, user: dict = Depends(get_or_create_pyinvest_user)):
...
async def deactivate_my_investor_account(user: dict = Depends(get_or_create_pyinvest_user)):
...
@customers.get('/all')
async def list_all_investors(user: dict = Depends(get_or_create_pyinvest_user)):
