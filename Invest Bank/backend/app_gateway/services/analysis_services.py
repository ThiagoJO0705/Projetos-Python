...
class AnalysisService:
    ...
    def get_portfolio_composition(investments: List[Dict[str, Any]], usd_rate: float = 1.0) -> List[Dict[str, Any]]:
        ...
        df = pd.DataFrame(investments)
        df['type'] = df['asset'].apply(AnalysisService._extract_type)
        df['currency'] = df['asset'].apply(AnalysisService._extract_currency)
        df['price'] = df['asset'].apply(AnalysisService._extract_price)
        
        # Calcula valor de cada linha e converte se necessário
        df['val'] = df['quantity'].astype(float) * df['price'].astype(float)
        df['current_value_brl'] = np.where(df['currency'] == 'USD', df['val'] * usd_rate, df['val'])
        
        composition = df.groupby('type')['current_value_brl'].sum().reset_index()
        return composition.to_dict(orient='records')
    
    @staticmethod
    def get_assets_performance(investments: List[Dict[str, Any]], usd_rate: float = 1.0) -> List[Dict[str, Any]]:
        ...
        df['val_market'] = df['quantity'].astype(float) * df['price'].astype(float)
        df['val_brl'] = np.where(df['currency'] == 'USD', df['val_market'] * usd_rate, df['val_market'])
        df['profit_loss_brl'] = df['val_brl'] - df['total_paid']
        performance = df.groupby('ticker')['profit_loss_brl'].sum().reset_index()
        return performance.to_dict(orient='records')
    
    @staticmethod
    def get_highlights(investments: List[Dict[str, Any]], usd_rate: float = 1.0) -> Dict[str, Any]:
        ...
        df['val_market'] = df['quantity'].astype(float) * df['price'].astype(float)
        df['val_brl'] = np.where(df['currency'] == 'USD', df['val_market'] * usd_rate, df['val_market'])
        df['profit'] = df['val_brl'] - df['total_paid']
    ...
    def compare_with_benchmark(portfolio_yield_pct: float, benchmark_df: pd.DataFrame) -> Dict[str, Any]:
        if benchmark_df.empty: return {'error': 'Benchmark indisponível'}
